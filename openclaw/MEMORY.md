...
- Prompt-library maintenance rule (2026-03-07): the canonical chooser lives in `workspace/prompts/openclaw/README.md`, and the pin-ready short version lives in `workspace/prompts/openclaw/DISCORD_PINNED_QUICKSTART.md`. When the Discord home-channel model, prompt selection, or OpenClaw upstream flow changes, update both together and re-sync/harden so the mirror and specialist prompt copies stay aligned.
